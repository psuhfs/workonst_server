    ...
    async auth(req: Request): Promise<CustomResponse> {
        try {
            ...
            return internalServerError(
                "An error occurred while trying to authenticate the request.",
                e.toString(),
            );
        }
        return success("Blah");
...
export class GetPointsHandler implements RequestHandler {
    async handle(_req: Request, _params: Record<string, string>): Promise<CustomResponse> {
        return prisma.test_Points.findMany().then((data) => {
            return success(data);
        }).catch((err) => {
            return internalServerError(`An error occurred while trying to fetch points: ${err}`);
        });
    }
