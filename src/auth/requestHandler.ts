...
export async function handleAuthSignup(req: Request): Promise<CustomResponse> {
    try {
        ...
        return internalServerError("Unable to process auth signup request.", e.toString());
    }
}
...
function genToken(body: AuthModel): Token {
    return {
        token: jwt.sign({pw: body.password, username: body.username}, process.env.JWT, {expiresIn: '10h'})
    }
}
